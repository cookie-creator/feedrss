  ...
  <page-component>
    <template v-slot:header>
      <div class="flex justify-between items-center">
        ...
        <router-link
          :to="{ name: 'PostCreate'}"
          class="py-2 px-3 text-white text-sm bg-emerald-500 hover:bg-emerald-600 rounded-lg">
    ...
    <div>
      <table class="w-full">
        ...
        <tr>
          ...
          <th class="p-3 text-sm font-light tracking-wide text-left">Created</th>
          <th class="p-3 text-sm font-light tracking-wide text-left"></th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(post, index) in posts.data" :key="post.title" class="bg-gray-50 hover:bg-gray-100">
          ...
          <td class="p-3 text-sm text-gray-700">
            <div>
              <button
                ...
                class="
                w-100 py-1 px-2 text-sm text-white bg-red-500 hover:bg-red-890 rounded-md">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 -mt-1 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<script setup>
  ...
  function getForPage(ev, link)
  {
    ...
    store.dispatch("getAuthPosts", { url: link.url });
  }
